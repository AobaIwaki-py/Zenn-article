...
---
# この記事の概要
この記事では、Prometheus、Node Exporter、Grafanaを利用したリッチなサーバー監視システムをミニマルに構成する方法を解説します。以下にPrometheus周りの構成図を示していますが、私が今回解説するのはオレンジ色の角丸四角形で囲まれた部分についてのみです。Alert Managerなどその他の機能を実装したい方は他の方の記事や公式ドキュメントをご覧ください。

![](/images/a612bf497c59ca/2023-03-11-17-59-59.png)

## Prometheusとは
- 公式ドキュメント : [Overview | Prometheus](https://prometheus.io/docs/introduction/overview/)

Prometheusとは、システムを監視したりアラートを飛ばしたりするためのオープンソースのツールキットです。Prometheusが収集したデータにアクセスするには、PromQLというクエリ言語を用います。しかし、PromQLを学習する必要性はあまりありません。というのも、後述するGrafanaのためのダッシュボードがネット上にすでに公開されているからです。それらを利用することで手軽にサーバーの情報を綺麗に可視化することができます。以下のサイトで構築済みのダッシュボードが多数公開されています。用途に合わせて利用してください。
- [Dashboards | Grafana Labs](https://grafana.com/grafana/dashboards/)

Prometheusは、監視対象からmetricsを収集してデータベースに保存します。metricsとは、数値で表される何らかの測定値のことです。

## Node Exporterとは
- 公式ドキュメント : [Monitoring Linux host metrics with the Node Exporter | Prometheus](https://prometheus.io/docs/guides/node-exporter/)

Node Exporterとは、Prometheusサーバーに監視対象のハードウェアやOSに関するmetricsを送信するためのPrometheus Exporterです。Prometheus Exporterとは、監視対象のmetricsをPrometheusが扱える形にして外部に送信するためのライブラリのことです。以下のサイトの一覧が掲載されています。
- [Exporters and integrations | Prometheus](https://prometheus.io/docs/instrumenting/exporters/)
また、よく使われるexporterは以下の記事にまとめられています。
- [よく使いそうなPrometheusのexporter一覧](https://zenn.dev/mttk030/articles/b4c85b02f7c750)

## Grafanaとは
- 公式ドキュメント : [Grafana | Query, visualize, alerting observability platform](https://grafana.com/grafana/)

Grafanaとは、Prometheusサーバーにクエリを投げてログやデータをダッシュボードに可視化するためのオープンソースツールです。クエリ言語であるPromQLを学ばなくとも公開されている構築済みのダッシュボードを用いることで簡単に洗練されたシステム管理ツールを構成できます。また、PromQLを勉強すれば目的に合わせたダッシュボードを構築することもできます。
...
- この記事では、デフォルトポートを以下のように設定しています。実装環境によってこれらのポートがすでに使用されている場合は、異なるポートを使用してください。
  - Prometheus : 9090
  - Node Exporter : 9100
  - Grafana : 3000
```yaml:prometheus.yml
...
- 上のようなすでに用意されたダッシュボードを利用するために次のような設定を行います。また、以下に公式のドキュメントを示します。
  - [Manage dashboards | Grafana documentation](https://grafana.com/docs/grafana/latest/dashboards/manage-dashboards/)
# 参考記事
