...
このセクションでは、`docker run`、`docker-compose up`、プレコンパイルされたバイナリファイルという3種類の方法でPrometheusの設定方法を解説します。一番やりやすい方法で設定することをお勧めします。

- `prometheus.yml` の中身は設定方法によらず同一です。
- この記事では、デフォルトポートを以下のように設定しています。
  - Prometheus : 9090
  - Node Exporter : 9100
```yaml:prometheus.yml
global:
