...
Node Exporterとは、Prometheusサーバーに監視対象のハードウェアやOSに関するmetricsを送信するためのPrometheus Exporterです。Prometheus Exporterとは、監視対象のmetricsをPrometheusが扱える形にして外部に送信するためのライブラリのことです。以下のサイトにExporterの一覧が掲載されています。

https://prometheus.io/docs/instrumenting/exporters/

また、よく使われるexporterは以下の記事にまとめられています。

https://zenn.dev/mttk030/articles/b4c85b02f7c750
...
```
- `prometheus.yml`は`~/prometheus/prometheus.yml`配置されているものとします。
...
$ sudo docker run -d --name prometheus -p 9090:9090 \
    ...
    prom/prometheus
```
- 本記事では、PrometheusサーバーがGrafanaサーバーを兼任するので、Grafanaもインストールします。
...
- 最後に、Loadして以下のような画面が表示されれば成功です。もしかするとLoad直後はデータが来ていない可能性もあるので、何も表示されなくてもしばらく待ってみてください。
